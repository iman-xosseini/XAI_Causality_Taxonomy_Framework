...
No features remained completely stable across counterfactuals. Most sensitive features: Rotational_speed (changed 100x), Air_temperature (changed 100x), wear_torque_product (changed 100x), Tool_wear (changed 100x), Temp_difference (changed 100x). Model predictions across counterfactuals: 0: 100. Model prediction is robust to most feature changes. Successfully completed 100/100 counterfactual runs.
...
The Tool Wear Failure is primarily driven by factors where tool wear increased significantly, climbing to a critical level between 200 and 240, triggering machine failure. The rotational speed significantly impacts tool wear, with an effect strength of 0.65, indicating that variations in speed contributed to increased wear. Additionally, the interaction between wear and torque (effect strength 0.6) exacerbates tool degradation. The robustness of predictions in counterfactual analysis highlights that adjustments in rotational speed and wear to torque notably impact system behavior, resulting in reduced tool lifespan and subsequent failure.

### b. Actionable Recommendation

Develop and implement hybrid TCN-BiLSTM models that track wear trends using vibration, force, and temperature signals for predictive maintenance. Ensure these models trigger real-time parameter optimization, particularly for feed and speed, to extend tool life. Counterfactual analysis shows rotational speed, wear torque, and temperature difference as highly sensitive features; therefore, adjust model inputs to track and modify these elements dynamically. Use robust model predictions to refine interventions and minimize failure risks, optimizing operational conditions to mitigate tool wear and extend machine uptime effectively.

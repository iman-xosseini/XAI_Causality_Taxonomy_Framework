...
No features remained completely stable across counterfactuals. Most sensitive features: wear_torque_product (changed 100x), Rotational_speed (changed 100x), Temp_difference (changed 100x), Tool_wear (changed 100x), Air_temperature (changed 100x). Model predictions across counterfactuals: 0: 100. Model prediction is robust to most feature changes. Successfully completed 100/100 counterfactual runs.
...
The Tool Wear Failure was primarily triggered by high tool wear, calculated at 0.83399209486166, and its interaction with rotational speed and wear-torque product. The causal relationships indicate that rotational speed significantly influences tool wear, with an effect strength of 0.65. Additionally, wear-torque product, a derived sensor metric, impacted tool wear with a strength of 0.6. Given these dynamics, excessive tool wear resulted in failure. Counterfactual analysis shows that the system is robust to feature changes, indicating a stable pattern leading to this failure.

### b. Actionable Recommendation

Implement a predictive maintenance system using hybrid TCN-BiLSTM models to forecast tool wear trends based on vibration, force, and temperature signals. Focus on data inputs related to wear-torque product and rotational speed, proven highly sensitive. These models should deploy in real-time to trigger parameter optimization of feed and speed settings, reducing the risk of future failures. Ensure the system can adapt parameter changes rapidly to extend tool life effectively. Use insights from counterfactual stability to fine-tune model robustness against feature variations.
